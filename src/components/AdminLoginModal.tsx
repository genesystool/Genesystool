...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Handle Firebase operation-not-allowed gracefully when Auth providers are disabled in console
      if (err.code === 'auth/operation-not-allowed') {
        try {
          await signInAnonymously(auth);
        } catch (e) {
          // Ignore anonymous fallback failure
        }
        setLoading(false);
        onSuccessLogin();
        return;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
        ...
        >
          ...
          {errorMsg && (
            <div className="my-4 p-3 bg-red-500/20 border border-red-500/40 rounded-2xl text-xs text-red-200 flex items-start gap-2">
